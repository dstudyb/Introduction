...
WidgetMetadata = {
    ...
    iconurl: "https://raw.githubusercontent.com/dstudyb/Introduction/main/icon/H-icon.png",
        ...
        },
        {
            ...
            params: [
                ...
                },
                {
                    ...
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_ATTR_ITEMS]
                },
                {
                    name: "tag_rel",
                    title: "人物关系",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_REL_ITEMS]
                },
                {
                    name: "tag_role",
                    title: "角色设定",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_ROLE_ITEMS]
                },
                {
                    name: "tag_body",
                    title: "外貌身材",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_BODY_ITEMS]
                },
                {
                    name: "tag_loc",
                    title: "情境场所",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_LOC_ITEMS]
                },
                {
                    name: "tag_plot",
                    title: "故事剧情",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_PLOT_ITEMS]
                },
                {
                    name: "tag_pos",
                    title: "性交体位",
                    type: "enumeration",
                    value: "all",
                    enumOptions: [ALL_OPT, ...TAG_POS_ITEMS]
                },
...
const NFO_INDEX_URL = "https://raw.githubusercontent.com/dstudyb/Introduction/main/index.json";
const IMAGE_INDEX_URL = "https://raw.githubusercontent.com/dstudyb/Introduction/main/manifest.json";
const INDEX_CACHE_TTL = 3600 * 1000;
...
const S2T_SORTED_KEYS = Object.keys(S2T_MAP).sort((a, b) => b.length - a.length);

function convertToTraditional(text) {
    if (!text) return "";
    let result = text;
    for (const simp of S2T_SORTED_KEYS) {
...
const T2S_MAP = {};
Object.keys(S2T_MAP).forEach(simp => {
    const trad = S2T_MAP[simp];
    if (trad && trad !== simp && !(trad in T2S_MAP)) {
        T2S_MAP[trad] = simp;
    }
});

const T2S_SORTED_KEYS = Object.keys(T2S_MAP).sort((a, b) => b.length - a.length);

function convertToSimplified(text) {
    if (!text) return "";
    let result = text;
    for (const trad of T2S_SORTED_KEYS) {
        if (result.includes(trad)) {
            result = result.split(trad).join(T2S_MAP[trad]);
        }
    }
    return result;
}
...
function normalizeMatchKey(text) {
    ...
    s = s.replace(/\u3000/g, " ");
    s = convertToSimplified(s);
    s = s.replace(/[^0-9a-z\u3040-\u30ff\u3400-\u9fff]/g, "");
    return s;
}

// 含日文假名（平假名/片假名）即视为日文原名
function hasKana(text) {
    return /[\u3040-\u30ff]/.test(text || "");
}

// 从网站抓到的多个标题中挑出中文名（不含假名者优先），用于匹配索引表
function pickChineseTitle(candidates) {
    const list = (Array.isArray(candidates) ? candidates : [candidates])
        .map(t => (t || "").trim())
        .filter(Boolean);
    return list.find(t => !hasKana(t)) || "";
}

const IMAGE_LOOKUP = {};
const NFO_LOOKUP = {};

function clearLookup(target) {
    Object.keys(target).forEach(k => delete target[k]);
}

function buildLookups(nfoData, imageData) {
    const nfoItems = (nfoData && Array.isArray(nfoData.items)) ? nfoData.items : [];
    const imageItems = (imageData && Array.isArray(imageData.images)) ? imageData.images : [];
    const nfoBase = (nfoData && nfoData.base) || "";

    clearLookup(IMAGE_LOOKUP);
    imageItems.forEach(item => {
        const key = normalizeMatchKey(item.name);
        if (key && !IMAGE_LOOKUP[key]) IMAGE_LOOKUP[key] = item.url;
    });

    clearLookup(NFO_LOOKUP);
    nfoItems.forEach(item => {
        const url = item.url || (nfoBase && item.file ? nfoBase.replace(/\/$/, "") + "/" + String(item.file).replace(/^nfo\//, "") : "");
        if (!url) return;
        const entry = { url, title: item.title, originaltitle: item.originaltitle };
        [item.title, item.originaltitle].concat(item.keys || []).forEach(k => {
            const key = normalizeMatchKey(k);
            if (key && !NFO_LOOKUP[key]) NFO_LOOKUP[key] = entry;
        });
    });
}

buildLookups(null, null);

const INDEX_STATE = { loadedAt: 0, loading: null };
...
async function ensureIndexes() {
    const now = Date.now();
    if (INDEX_STATE.loadedAt && now - INDEX_STATE.loadedAt < INDEX_CACHE_TTL) return;
    if (INDEX_STATE.loading) return INDEX_STATE.loading;
    INDEX_STATE.loading = (async () => {
        try {
            const [nfoData, imageData] = await Promise.all([
                fetchIndexJson(NFO_INDEX_URL),
                fetchIndexJson(IMAGE_INDEX_URL)
            ]);
            buildLookups(nfoData, imageData);
        } finally {
            INDEX_STATE.loadedAt = Date.now();
            INDEX_STATE.loading = null;
        }
    })();
    return INDEX_STATE.loading;
}

function lookupByTitles(lookup, titles) {
...
function matchIndexImage(titles) {
    const direct = lookupByTitles(IMAGE_LOOKUP, titles);
    if (direct) return direct;
    const nfo = matchNfoEntry(titles);
    if (nfo) {
        const bridged = lookupByTitles(IMAGE_LOOKUP, [nfo.title]);
        if (bridged) return bridged;
    }
    return "";
}

function matchNfoEntry(titles) {
    return lookupByTitles(NFO_LOOKUP, titles) || null;
}

async function fetchNfoPlot(url) {
    if (!url) return "";
    try {
        const response = await Widget.http.get(url, {
            headers: getCommonHeaders(BASE_URL + "/"),
        ...
        const match = xml.match(/<plot>\s*(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?\s*<\/plot>/i);
        if (!match) return "";
        const plot = match[1].replace(/<!\[CDATA\[|\]\]>/g, "").replace(/\s+/g, " ").trim();
        return plot ? convertToTraditional(plot) : "";
    } catch (e) {
        return "";
    }
}

async function preCacheNfo(items) {
    const promises = [];
    for (const item of items) {
        if (!item.link) continue;
        const key = "nfo:" + item.link;
        if (Widget.storage.get(key)) continue;
        const cn = pickChineseTitle([item.title]);
        if (!cn) continue;
        const nfo = matchNfoEntry([cn]);
        if (!nfo) continue;
        promises.push(
            fetchNfoPlot(nfo.url).then(function (plot) {
...
function parseListHtmlCore(html, mode) {
    ...
    buildCards($).forEach($card => {
        ...
        const finalImage = matchIndexImage(pickChineseTitle([title])) || normUrl;
        items.push({
            id: link,
            type: "url",
            title: convertToTraditional(title),
            posterPath: (mode === "poster" || mode === "both") ? finalImage || undefined : undefined,
            backdropPath: (mode === "backdrop" || mode === "both") ? finalImage || undefined : undefined,
            durationText,
            releaseDate,
            link,
            mediaType: "movie",
...
function parseListHtml(html)         { return parseListHtmlCore(html, "backdrop"); }
function parseListHtmlPortrait(html) { return parseListHtmlCore(html, "poster"); }
function parseListHtmlSearch(html)   { return parseListHtmlCore(html, "both"); }

async function fetchAndParse(url, referer, parser = parseListHtml) {
    try {
        await ensureIndexes();
        const response = await httpGetWithTimeout(url, referer);
        const items = parser(response.data || "");
        return preCacheNfo(items);
    } catch (e) {
        return [];
    }
}
...
async function searchVideos(params) {
    ...
    const sort = SORT_MAP[params.sort_by || 'all'] || "";
    const queryParts = [];
...
async function loadAdvancedGenre(params) {
    ...
    const genre = GENRE_MAP[params.genre] || "";
    const sort = SORT_MAP[params.sort_by || "all"] || "";
    const time = TIME_MAP[params.upload_time] || "";
    const duration = DURATION_MAP[params.duration_filter] || "";
    const queryParts = [];
...
async function loadHotRankings(params) {
    ...
    const jumped = await handleJumpUrl(params, page);
    if (jumped) return jumped;
    const sortVal = SORT_MAP[params.sort || "watching"] || "他們在看";
    const queryParts = [];
...
async function loadByTag(params) { const page = parseInt(params.page)||1; const j = await handleJumpUrl(params,page); if(j) return j; const { url, referer } = buildTagModuleUrl(params); return fetchAndParse(url, referer); }
const loadByTagAttr = loadByTag;
const loadByTagRel  = loadByTag;
const loadByTagRole = loadByTag;
const loadByTagBody = loadByTag;
const loadByTagLoc  = loadByTag;
const loadByTagPlot = loadByTag;
const loadByTagPos  = loadByTag;
...
async function loadDetail(link) {
    try {
        ...
        let matchTitles = [];
        const storedNfo = Widget.storage.get("nfo:" + link);
        if (storedNfo) {
            if (storedNfo.title) {
                title = convertToTraditional(storedNfo.title);
                tagline = storedNfo.originaltitle
                    ? convertToTraditional(storedNfo.originaltitle)
                    : (origTitle ? convertToTraditional(origTitle) : tagline);
            }
            if (storedNfo.plot) description = storedNfo.plot;
        } else {
            const siteCnTitle = pickChineseTitle([
                origTitle,
                transTitle,
                safeText($('meta[property="og:title"]').attr('content')),
                safeText($('title').text()).replace(/[-|–—_]\s*hanime1.*$/i, '').trim(),
                title
            ]);
            if (siteCnTitle) matchTitles = [siteCnTitle];
            const nfoEntry = matchNfoEntry(matchTitles);
            if (nfoEntry) {
                const nfoPlot = await fetchNfoPlot(nfoEntry.url);
                if (nfoPlot) description = nfoPlot;
                if (nfoEntry.title) {
                    title = convertToTraditional(nfoEntry.title);
                    tagline = nfoEntry.originaltitle
                        ? convertToTraditional(nfoEntry.originaltitle)
                        : (origTitle ? convertToTraditional(origTitle) : tagline);
                }
            }
        }
        ...
        const indexImage = matchIndexImage(matchTitles);
        let finalPosterPath = indexImage || undefined;
        let finalBackdropPath = indexImage || ogImage || undefined;
        ...
        if ($playlistWrapper.length) {
            ...
            $playlistScroll.find('.related-watch-wrap.multiple-link-wrapper').each((idx, el) => {
                ...
                const epCover = matchIndexImage(pickChineseTitle([epTitle])) || normalizeImageUrl(epPosterRaw) || undefined;
                ...
                seriesItems.push({
                    id: epLink,
                    type: 'url',
                    title: convertToTraditional(epTitle),
        ...
        // 多集系列聚合成选集列表；单集保持单集
        const episodeItems = seriesItems.length > 1
            ? seriesItems.map((it, idx) => ({ ...it, episodeNumber: idx + 1 }))
            : [];
        ...
        relatedCards.forEach($card => {
            ...
            const recBackdrop = matchIndexImage(pickChineseTitle([recTitle])) || normalizeImageUrl(recPosterRaw) || undefined;
